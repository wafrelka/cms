...
{% for task in user["tasks"] %}
	...
	{% for testcase in task["testcases"] %}
		{% set text_present = testcase["text"] != "N/A" %}
		{% try %}
			{% set mem = format_size(testcase["execution_memory"]).replace(" ","") %}
		{% except %}
			{% set mem = "N/A" %}
		{% end %}
		{% try %}
			{% set exec_time = "%.3fs" % testcase["execution_time"] %}
		{% except %}
			{% set exec_time = "N/A" %}
		{% end %}
		\item[{{ testcase["name"] }}]
		\makebox[8mm]{ {{ testcase["text"] }} } {% if text_present %}
		\makebox[14mm]{ {{ exec_time }} }
		\makebox[14mm]{ {{ mem }} }
		{% end %}
	{% end %}
	\end{description}
